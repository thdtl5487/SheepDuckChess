...
const OverlayEffects = ({
    ...
    const attackerImgStyle = {
        transform: isOpponentAttack ? "scaleX(-1)" : undefined,
    };
    const victimImgStyle = {
        filter: isOpponentAttack ? "brightness(0.5)" : undefined,
        transform: isOpponentAttack ? undefined : "scaleX(-1)",
    };
    ...
    return (
        <div className="absolute inset-0 z-40 pointer-events-none">
            ...
            {attackerImage && (
                <motion.img
                    src={`${attackerImage}?t=${isVisible ? Date.now(): ''}`}
                    alt="attacker"
            ...
            {victimImage && (
                <motion.img
                    src={`${victimImage}?t=${isVisible ? Date.now(): ''}`}
                    alt="victim"
                    initial={{ scale: 1 }}
                    animate={{
                        scale: [1, 1.1, 0.9, 1],
